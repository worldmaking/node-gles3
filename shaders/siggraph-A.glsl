...
void mainImage( out vec4 OUT, in vec2 COORD )
{
    ...
    float blend = 1.;  // I like blend=0 more, it gives more turbulence; 1 is more smoky
    //OUT.xy = avg.xy + force;
    ...
    */
    ...
    // optional add forces
    vec2 ctr = vec2(0, DIM.y/2.);
    float d = line2(COORD, 
        ctr - DIM.y*0.2* vec2(sin(iTime*0.2),cos(iTime*0.2)), 
        ctr + DIM.y*0.4* vec2(sin(iTime*.1618),cos(iTime*.1618))
    );
    if (d < 1.) {
    ...
    // boundary:
    float b = 4.;
    if (int(iFrame) % 4800 == 0 || COORD.x < b || COORD.y < b || DIM.x-COORD.x < b || DIM.y-COORD.y < b) {
        OUT = vec4(1., 0., 0., 0.);
    }
