...
struct Hmd {
    ...
    void configure() { 
		...
		fprintf(stdout, "VR display %s driver %s", mDisplay.c_str(), mDriver.c_str());
	...
	//	fbo.dim.x = w;
	//	fbo.dim.y = h;
		fprintf(stdout, "VR texture recommendation %d x %d\n", w, h);
    ...
    void update(glm::mat4 modelview_mat = glm::mat4()) {
		...
		while (mHMD->PollNextEvent(&event, sizeof(event))) {
			...
			break;
			//default: fprintf(stdout, "htcvive event %d", event.eventType);
			}
		}
		...
		if (err != vr::VRCompositorError_None) {
			fprintf(stderr, "WaitGetPoses error");
			return;
		}
...
// 	void submit(GLuint srcFBO, GLuint srcDimX, GLuint srcDimY) {
// #ifdef USE_STEAM_DRIVER
// 		if (!mHMD) return;
// 		if (!fbo.fbo) {
// 			fprintf(stdout, "no HMD FBO/texure yet");
...
// 		glBlitFramebuffer(0, 0, srcDimx, srcDimy, 0, 0, fbo.dim.x, fbo.dim.y, GL_COLOR_BUFFER_BIT,GL_LINEAR);
...
// 	}
    ...
    bool submit(GLuint texid) {
        bool ok = false;
#ifdef USE_STEAM_DRIVER
		if (!mHMD) return false;
		if (!texid) {
			fprintf(stdout, "no HMD FBO/texure yet");
		...
		vr::Texture_t vrTexture = { (void *)texid, vr::TextureType_OpenGL, vr::ColorSpace_Gamma };
		...
		case 0:
			break;
		case 1:
			fprintf(stdout, "submit error: Request failed.");
			break;
		case 100:
			fprintf(stdout, "submit error: Incompatible version.");
			break;
		case 101:
			fprintf(stdout, "submit error: Do not have focus.");
			break;
		case 102:
			fprintf(stdout, "submit error: Invalid texture.");
			break;
		case 103:
			fprintf(stdout, "submit error: Is not scene application.");
			break;
		case 104:
			fprintf(stdout, "submit error: Texture is on wrong device.");
			break;
		case 105:
			fprintf(stdout, "submit error: Texture uses unsupported format.");
			break;
		case 106:
			fprintf(stdout, "submit error: Shared textures not supported.");
			break;
		case 107:
			fprintf(stdout, "submit error: Index out of range.");
			break;
		case 108:
			fprintf(stdout, "submit error: Already submitted.");
			break;
		default:
			fprintf(stdout, "submit error: ???.");
			break;
		}
    ...
    bool dest_changed() {
		//fprintf(stdout, "HMD dest_changed %d %d", fbo.dim.x, fbo.dim.y);
